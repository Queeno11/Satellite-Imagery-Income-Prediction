...
def create_generators(
    ...
    train_generator = train_generator.flow_from_dataframe(
        ...
        target_size=(244, 244),
        batch_size=0,  # increase or decrease to fit your GPU
    )

    validation_generator = validation_generator.flow_from_dataframe(
        dataframe=val, x_col="image", y_col="rmax", class_mode="raw", target_size=(244, 244), batch_size=128,
    )
    test_generator = test_generator.flow_from_dataframe(
        dataframe=test, x_col="image", y_col="rmax", class_mode="raw", target_size=(244, 244), batch_size=128,
    )
...
def get_callbacks(model_name: str) -> List[Union[TensorBoard, EarlyStopping, ModelCheckpoint]]:
    ...
    model_checkpoint_callback = ModelCheckpoint(
        "{path_dataout}/models/" + model_name + ".h5",
        monitor="val_mean_squared_logarithmic_error",
        verbose=0,
...
def run_model(
    ...
    model.compile(
        optimizer=optimizer, loss="mean_squared_error", metrics=[MeanAbsolutePercentageError(), MeanSquaredError(), MeanSquaredLogarithmicError()]
    )
    history = model.fit(
        train_generator,
        epochs=100,
...
def plot_results(model_history_small_cnn: History, model_history_eff_net: History, mean_baseline: float):
    ...
    dict1 = {
        "MAPE": model_history_small_cnn.history["mean_squared_logarithmic_error"],
        "type": "training",
        "model": "small_cnn",
    }
    dict2 = {
        "MAPE": model_history_small_cnn.history["val_mean_squared_logarithmic_error"],
        "type": "validation",
        "model": "small_cnn",
    }
    dict3 = {
        "MAPE": model_history_eff_net.history["mean_squared_logarithmic_error"],
        "type": "training",
        "model": "eff_net",
    }
    dict4 = {
        "MAPE": model_history_eff_net.history["val_mean_squared_logarithmic_error"],
        "type": "validation",
        "model": "eff_net",
    }
...
def run(small_sample=False):
    ...
    if small_sample == True:
        df = df.iloc[0:100]  # set small_sampe to True if you want to check if your code works without long waiting
    ...
    # FIXME: effnet not working. I will try to fix it later.
    ...
    plot_results(small_cnn_history, small_cnn_history, mean_baseline)
