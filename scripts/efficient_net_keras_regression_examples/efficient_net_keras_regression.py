...
def get_callbacks(model_name: str) -> List[Union[TensorBoard, EarlyStopping, ModelCheckpoint]]:
    ...
    model_checkpoint_callback = ModelCheckpoint(
        "R:/Tesis Nico/Códigos/data/data_out/efficient_net_keras_regression/models/" + model_name + ".h5",
        monitor="val_mean_absolute_percentage_error",
        verbose=0,
...
def run(small_sample=False):
    ...
    df = pd.read_pickle("R:/Tesis Nico//Códigos/data/data_out/efficient_net_keras_regression/df.pkl")
    df["image_location"] = (
        "R:/Tesis Nico//Códigos/data/data_out/efficient_net_keras_regression/processed_images/" + df["zpid"] + ".png"
